...
export default function CollaborationProcess({ locale }: { locale: Locale }) {
  ...
  // Step progress transforms (3 steps)
  const step1Progress = useTransform(scrollYProgress, [0, 0.33], [0.3, 1], { clamp: true });
  const step2Progress = useTransform(scrollYProgress, [0.33, 0.66], [0.3, 1], { clamp: true });
  const step3Progress = useTransform(scrollYProgress, [0.66, 1], [0.3, 1], { clamp: true });
  ...
  const step1Slide = useTransform(scrollYProgress, [0, 0.33], [20, 0], { clamp: true });
  const step2Slide = useTransform(scrollYProgress, [0.33, 0.66], [20, 0], { clamp: true });
  const step3Slide = useTransform(scrollYProgress, [0.66, 1], [20, 0], { clamp: true });

  return (
    <section ref={sectionRef} id="how-it-works" className="relative bg-[#0a0c0a] py-24 px-4">
      <div className="mx-auto max-w-4xl px-6 md:px-12">
        <motion.h2
          ...
          className="mb-12 text-sm font-mono uppercase tracking-wider text-zinc-400"
        >
